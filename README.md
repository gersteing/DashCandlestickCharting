...
This project requires you install python. This was developed with python 3.6.5. The only other dependencies I installed were Dash, plotly, pytz, and pandas. Need help to install or understand these libraries better to make sense of this example? See section below - Installing and Getting Help with the Dependent Libraries

The only other thing that might trip you up when running this example is if you try to run the script from the idle python editor. You cannot start a flask server from within the idle python editor application so openning the file in idle and hitting Run will cause an error. 
...
4) Then open a browser at the following address: 
http:127.0.0.1:8050/ 
You will see the candlestick chart in your web browser.
